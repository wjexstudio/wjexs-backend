...
import { fetchProjects, getRepoIssues, createRepoIssue, updateRepoIssue } from '../services/githubService';
...
router.get('/', async (req, res) => {
  try {
    ...
    if (projectId) {
      reposToFetch.push(projectId as string);
    } else {
    ...
    const promises = reposToFetch.map(async (repo) => {
      try {
        ...
        return onlyIssues.map((issue: any) => ({
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!resGithub.ok) throw new Error('Failed to create issue');
...
router.patch('/:id/mode', async (req, res) => {
  try {
    ...
    if (!repo || !issueNumber) return res.status(400).json({ error: 'Invalid quest ID format (expected repo__number)' });
    ...
    const existingLabels = issue.labels.map((l: any) => l.name).filter((n: string) => !n.startsWith('mode:'));
