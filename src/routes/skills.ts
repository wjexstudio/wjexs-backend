...
import { getRepoFile } from '../services/githubService';
...
router.get('/', async (req, res) => {
  try {
    ...
    if (!resGithub.ok) throw new Error('Failed to fetch skills directory');
    ...
    for (const item of items) {
      if (item.type === 'dir') {
        ...
        if (name.includes('memory') || name.includes('session')) cluster_name = 'Memory';
        else if (name.includes('caveman')) cluster_name = 'Communication';
        else if (name.includes('team') || name.includes('orchestration') || name.includes('crew')) cluster_name = 'Core Logic';
